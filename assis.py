...
import difflib  # Added for similarity matching in training
...
class UltimateVoiceAssistant:
    ...
    def init_databases(self):
        ...
        tables = [
            ...
            '''CREATE TABLE IF NOT EXISTS training_data (
                ...
                intended_command TEXT NOT NULL
            )'''
        ]
    ...
    def process_enhanced_command(self, command):
        ...
        try:
            ...
            if command in quick_responses:
                ...
                # Process command with learned mappings
    ...
    def match_and_execute_command(self, command):
        ...
        # Check for learned command mappings first
        trained_command = self.get_trained_command(command)
        if trained_command:
            command = trained_command  # Override with learned command
    ...
    def add_training_data(self, input_text, intended_command):
        """Add training data for command correction"""
        try:
            conn = sqlite3.connect('personal_assistant.db')
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO training_data (input_text, intended_command) VALUES (?, ?)",
                (input_text, intended_command)
            )
            conn.commit()
            conn.close()
            logging.info(f"Added training data: '{input_text}' -> '{intended_command}'")
        except Exception as e:
            logging.error(f"Failed to add training data: {e}")

    def get_trained_command(self, command):
        """Get best matching trained command based on similarity"""
        try:
            conn = sqlite3.connect('personal_assistant.db')
            cursor = conn.cursor()
            cursor.execute("SELECT input_text, intended_command FROM training_data")
            ...
            for input_text, intended in data:
                similarity = difflib.SequenceMatcher(None, command.lower(), input_text.lower()).ratio()
                if similarity > best_similarity and similarity > 0.75:  # Adjustable threshold
                    best_similarity = similarity
                    best_intended = intended

            if best_intended:
                logging.info(f"Found trained mapping for '{command}' -> '{best_intended}' (similarity: {best_similarity:.2f})")
            return best_intended
        except Exception as e:
            logging.error(f"Failed to get trained command: {e}")
            return None

    def ai_fallback_response(self, command):
        """AI-powered fallback for unrecognized commands with training"""
        response = "I didn't understand that command. What did you mean?"
        self.speak(response)
        
        intended = simpledialog.askstring("Command Correction", "What command did you intend to say?\n(Leave blank if not sure)")
        
        if intended:
            self.add_training_data(command, intended)
            response += f"\nLearned: '{command}' maps to '{intended}'"
            # Execute the intended command
            self.process_enhanced_command(intended)
